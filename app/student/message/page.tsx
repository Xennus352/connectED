...
const StudentMessage = () => {
  ...
  // fetch unread messages count per user and listen to new messages
  useEffect(() => {
  ...
  const renderUserList = (role: string) => {
    ...
    return (
      <div className="grid gap-2 h-[calc(100vh-200px)] overflow-y-auto p-2">
        {filtered.map((u) => (
          <button
            key={u.id}
          ...
          >
            <div className="avatar relative">
              ...
              {unreadCounts[u.id] > 0 && (
                <span className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-500 animate-pulse rounded-full"></span>
              )}
            </div>
